...
def github_callback(request):  # Github
    ...
    data = {
        ...
        'client_id': '3dc2bdb6d5674b71f844',
        'client_secret': 'f7719d7595fd9ca8233a4c39b6e28ea36f319392',
    }
    response = requests.post(backend.ACCESS_TOKEN_URL, data=data)
    access_token = response.json()['access_token']

    # Use the access_token to fetch user information from the GitHub API
    user_response = requests.get('https://api.github.com/user', headers={
        'Authorization': f'token {access_token}',
    })
    user_data = user_response.json()

    # Process the user_data and log in the user
